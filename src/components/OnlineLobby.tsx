...
export function OnlineLobby({ onGameStart: handleGameStart, onBackToLocal, language }: OnlineLobbyProps) {
  ...
  useEffect(() => {
    ...
    onPlayerLeft((color) => {
      ...
      setError(`${color === 'black' ? labels.black : labels.white} ${labels.playerLeft}`);
    });
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      if (result.success && result.roomId) {
        ...
        setError(result.error || (language === 'ja' ? 'ルーム作成に失敗しました' : 'Failed to create room'));
      }
    } catch {
      setError(language === 'ja' ? '接続エラー' : 'Connection error');
    }
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      if (result.success && result.roomId) {
        ...
        setError(result.error || (language === 'ja' ? 'ルーム参加に失敗しました' : 'Failed to join room'));
      }
    } catch {
      setError(language === 'ja' ? '接続エラー' : 'Connection error');
    }
  ...
  return (
    <div className="h-screen bg-gray-900 flex flex-col items-center justify-center p-3 sm:p-4 overflow-hidden">
      <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6">{t.onlinePlay}</h1>

      {status === 'in-room' && roomId ? (
        // 待機画面
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg text-center">
          <h2 className="text-lg sm:text-xl text-white mb-3">{t.waiting}</h2>
          <div className="bg-gray-700 p-3 rounded mb-3">
            <p className="text-gray-400 text-xs sm:text-sm">{t.roomId}:</p>
            <p className="text-xl sm:text-2xl font-mono text-yellow-400">{roomId}</p>
          </div>
          <p className="text-gray-400 text-sm mb-3">
            {t.you}: <span className="font-bold text-white">{playerColor}</span>
          </p>
          <p className="text-gray-500 text-xs">{t.shareId}</p>
        </div>
      ) : (
        // ロビー画面
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg w-full max-w-sm sm:max-w-md">
          ...
          <div className="mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl text-white mb-3">{t.createRoom}</h2>
            <button
          ...
          {/* ルーム参加 */}
          <div>
            <h2 className="text-lg sm:text-xl text-white mb-3">{t.joinRoom}</h2>
            <input
              type="text"
              value={joinRoomId}
